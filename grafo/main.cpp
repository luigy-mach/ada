...
void cargar_grafo(Grafo<int> &grafo1)
{
 /*   grafo1.agregarVertice(1);
    ...
    grafo1.agregarArco(7,1,2);
    grafo1.agregarArco(8,1,2);
    grafo1.agregarArco(1,8,2);
*/
	srand(time(NULL));
	int tam_rand=10;
	int num=25;
	...
	for(int i=1;i<=num;i++)
	  for(int j=i;j<=num;j++)
	     if(i!=j)
		grafo1.agregarArco(i,j,rand()%tam_rand+1);
...
void print_dot(Grafo<int>& grafo, list<int>& vertices_grafo,estructura_etiqueta etiqueta[] )
{
	...
	while (v != vertices_grafo.end())
	{
		...
		while (ady != adyacentes.end())
		{
			...
			dot_file << *v << " -> "<< ady->devolverAdyacente() << "[label=\"" << ady->costo << "\"";

			if (etiqueta[ady->devolverAdyacente()].padre==*v)
				dot_file << ", color=\"red\"";
			else
				dot_file << ", color=\"grey\"";
				dot_file << "]\n";
...
int main()
{
   ...
   //----------------------------------------------------------------------------	
    ...
    print_dot(grafo1,vertices,etiqueta);
